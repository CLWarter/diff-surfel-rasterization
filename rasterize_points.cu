	...
	const bool debug) 
{
  ...
  torch::Tensor dL_dambients = torch::zeros({P, 1}, means3D.options());
  ...
  return std::make_tuple(dL_dmeans2D, dL_dcolors, dL_dopacity, dL_dambients, dL_dmeans3D, dL_dtransMat, dL_dsh, dL_dscales, dL_drotations);
}
